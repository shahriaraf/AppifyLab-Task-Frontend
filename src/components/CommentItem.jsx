...
const CommentItem = ({ comment, postId, onReplySubmit, onChildReplyAdd }) => {
  ...
  const renderBubbleIcons = () => {
    ...
    if (iconsToShow.length === 0 && reaction) iconsToShow = [reaction];
    else if (iconsToShow.length === 0) iconsToShow = ['Like'];

    if (reaction && !iconsToShow.includes(reaction)) {
      iconsToShow = [reaction, ...iconsToShow].slice(0, 2);
    }

    const primaryType = iconsToShow[0];
    const secondaryType = iconsToShow[1];
  ...
  const handlePressEnd = () => {
    ...
    if (!longPressRef.current && !showPicker) {
  ...
  const handleReaction = async (type) => {
    ...
    if (reaction === type) {
      ...
      setTopReactions((prev) => {
        const filtered = prev.filter((t) => t !== reaction);
        return [type, ...filtered].slice(0, 2);
      });
    }
      ...
      <div className="_comment_main" style={{ display: 'flex', alignItems: 'flex-start', position: 'relative', zIndex: 1 }}>
        ...
        <div className="_comment_bubble_wrapper" style={{ maxWidth: 'calc(100% - 50px)' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', fontSize: '12px', color: '#65676b', fontWeight: 'bold', margin: '15px 0 0 12px' }}>
            ...
            >
              {showPicker && (
                <div onMouseEnter={() => hoverTimerRef.current && clearTimeout(hoverTimerRef.current)}>
                  <ReactionPicker onSelect={handleReaction} onClose={() => setShowPicker(false)} />
                </div>
              )}
              {reactionStyle.label}
            </span>
            <span onClick={() => setShowReplyInput(!showReplyInput)} style={{ cursor: 'pointer' }}>
              Reply
            </span>
        ...
        <div style={{ paddingLeft: isReply ? '32px' : '40px', marginTop: '5px' }}>
          <form onSubmit={handleReplySubmit} style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <input
              ...
              style={{ width: '100%', height: '32px', padding: '0 10px', fontSize: '13px', borderRadius: '16px', background: '#f0f2f5', border: 'none', outline: 'none' }}
              autoFocus
            />
          </form>
        </div>
        ...
        <div style={{ paddingLeft: '40px', marginTop: '5px' }}>
          ...
          {showReplies && (
            <div style={{ paddingLeft: '40px', marginTop: '8px' }}>
              {replies.map((r) => (
                <CommentItem key={r._id} comment={r} postId={postId} onReplySubmit={onReplySubmit} onChildReplyAdd={handleChildNewReply} />
              ))}
            </div>
          )}
        </div>
